...
// net objects for the station and access point
 ...
 * @param event_id is the id of the event to resgister the handler for
...
/**
 * Initialzes the WiFi application event handler for WiFi and IP events
 */
...
{
    // Initialize tge TCP stack
    ESP_ERROR_CHECK(esp_netif_init());

    // Default WiFi config: operation must be done in this order
...
void wifi_app_soft_ap_config(void)
{
    ...
    esp_netif_dhcpc_stop(esp_netif_ap);             ///> must call this first
    inet_pton(AF_INET, WIFI_AP_IP, &ap_ip_info.ip); ///> Assigns  acces point's static IP, GW and netmask
    ...
    ESP_ERROR_CHECK(esp_netif_dhcpc_start(esp_netif_ap));               ///> Starts the AP DHCP server for connection station e.g. your mobile device

    ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_APSTA));                ///> Settings the mode as Access Point / Station Mode
    ESP_ERROR_CHECK(esp_wifi_set_config(ESP_IF_WIFI_AP, &ap_consig));   ///> Set our configurTION
...
{
    ESP_LOGI(TAG, "STARTING_WIFI_APPLICATION");
